module GrablyPkg
  ...
  class LibRangeParams # :nodoc:
    ...
    def initialize(l)
      ...
      m = /(.+)-([^-]+)/.match(l)
      if l.start_with?('~')
        version = VersionRange.new(m[2])
        l = m[1][1..-1]
      elsif l.start_with?('=')
        version = VersionRange.new_e(m[2])
        l = m[1][1..-1]
      elsif l.start_with?('>=')
        version = VersionRange.new_ge(m[2])
        l = m[1][2..-1]
      elsif l.start_with?('>')
        version = VersionRange.new_g(m[2])
        l = m[1][1..-1]
      elsif l.start_with?('<=')
        version = VersionRange.new_le(m[2])
        l = m[1][2..-1]
      elsif l.start_with?('<')
        version = VersionRange.new_l(m[2])
        l = m[1][1..-1]
      end
